...
module.exports = class ApiGatewayController {
  ...
  async takeOrders(req, res) {
    ...
    console.log(
      `API Gateway received ${
        orders.length
      } new pizza orders at ${ordersReceivedAt.toLocaleTimeString()}`
    );
    ...
    try {
      ...
      console.log(
        ...
        } pizza orders at ${ordersCompletedAt.toLocaleTimeString()}`
      );
    } catch (error) {
  ...
  prepareDoughRequest(order) {
    const reqOptions = {
      url: "http://dough:3001/prepareDough",
      method: "post",
  ...
  generateReport(completedOrders, ordersReceivedAt, ordersCompletedAt) {
    const report = {
      totalPreperationTime:
        (ordersCompletedAt.getTime() - ordersReceivedAt.getTime()) / 1000,
